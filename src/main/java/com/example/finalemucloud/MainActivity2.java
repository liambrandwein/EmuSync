...
public class MainActivity2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter.setOnItemClickListener(new EmulatorAdapter.OnItemClickListener() {
            ...
            public void onItemClick(Emulator emulator) {
                Intent intent = new Intent(MainActivity2.this, AddEditEmulatorActivity.class);
            ...
            public void onBackupClick(Emulator emulator) {
                if (!emulator.isMainUpload()) {
                    Toast.makeText(getApplicationContext(), "Error: make sure \"Sync directory contents\" is checked for the main directory",Toast.LENGTH_LONG).show();
                    try {
                        backupDialog.dismiss();
                    } catch (IllegalStateException e) {}
                    return;
                } else if (!directoryOk(emulator.getDirectory1())) {
                    return;
                }
                ...
                if (!emulator.isSecondaryUpload()) {
                    try {
                        backupDialog.dismiss();
                    } catch (IllegalStateException e) {}
                    return;
                } else if (!directoryOk(emulator.getDirectory2())) {
                    try {
                        backupDialog.dismiss();
                    } catch (IllegalStateException e) {}
                    return;
                    ...
                    public void onSuccess(HashMap<String, String> stringStringHashMap) {
                        if (!stringStringHashMap.containsKey(emulator.getTitle())) {
                            driveServiceHelper.insertFolderFolder(emulator.getTitle(), stringStringHashMap.get("EmuSync")).addOnSuccessListener(new OnSuccessListener<String>() {
                                @Override
                                public void onSuccess(String s) {
                                    driveServiceHelper.createFileV3(emulator.getDirectory2(), s).addOnSuccessListener(new OnSuccessListener<String>() {
                                        @Override
                                        public void onSuccess(String s) {
                                            Toast.makeText(getApplicationContext(), "Success",Toast.LENGTH_LONG).show();
                                        }
                            ...
                            driveServiceHelper.createFileV3(emulator.getDirectory2(),stringStringHashMap.get(emulator.getTitle())).addOnSuccessListener(new OnSuccessListener<String>() {
                                @Override
                                public void onSuccess(String s) {
                                    Toast.makeText(getApplicationContext(), "Success",Toast.LENGTH_LONG).show();
                                }
                ...
                driveServiceHelper.getAllFolders().addOnSuccessListener(new OnSuccessListener<HashMap<String, String>>() {
                    ...
                    public void onSuccess(HashMap<String, String> stringStringHashMap) {
                        if (!stringStringHashMap.containsKey(emulator.getTitle())) {
                            ...
                            String[] tmp = emulator.getDirectory2().split("/");
                            String tmp2 = tmp[tmp.length-1];
                            driveServiceHelper.downloadFile3(stringStringHashMap.get(tmp2), emulator.getDirectory2(), 1).addOnSuccessListener(new OnSuccessListener<String>() {
                                @Override
                                public void onSuccess(String s) {
                                    Toast.makeText(getApplicationContext(),"Success",Toast.LENGTH_LONG).show();
                                }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == ADD_EMULATOR_REQUEST && resultCode == RESULT_OK) {
            ...
            boolean mainUpload = data.getBooleanExtra(AddEditEmulatorActivity.EXTRA_MAINUPLOAD, true);
            boolean secondaryUpload = data.getBooleanExtra(AddEditEmulatorActivity.EXTRA_SECONDARYUPLOAD, false);
            int priority = data.getIntExtra(AddEditEmulatorActivity.EXTRA_PRIORITY, 2);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menuInflater.inflate(R.menu.home_layout_menu, menu);
        return true;
    }

    //"HAS_BEEN_INITIALIZED" changed
    @Override
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.refresh_button:
                refresh();
                return true;
            case R.id.global_settings_button:
                Intent intent2 = new Intent(MainActivity2.this, EmulatorConfigure.class);
                startActivity(intent2);
                return true;
            case R.id.add_emulator_button:
                Intent intent = new Intent(MainActivity2.this, AddEditEmulatorActivity.class);
                startActivityForResult(intent, ADD_EMULATOR_REQUEST);
                return true;
            default:
                ...
                .addOnSuccessListener(new OnSuccessListener<GoogleSignInAccount>() {
                    ...
                    public void onSuccess(GoogleSignInAccount googleSignInAccount) {
                        ...
                        GoogleAccountCredential credential = GoogleAccountCredential.
                                usingOAuth2(MainActivity2.this, Collections.singleton(DriveScopes.DRIVE_FILE));
    ...
    private boolean directoryOk(String directory) {
        File testFile = new File(directory);

        if (!testFile.exists()) {
            Toast.makeText(getApplicationContext(), "Error: directory does not exist: " + directory, Toast.LENGTH_LONG).show();
            return false;
        }
        else if (!testFile.isDirectory()) {
            Toast.makeText(getApplicationContext(), "Error: not a directory: " + directory, Toast.LENGTH_LONG).show();
            return false;
        }
        else if (directory.length() <= 20 && directory.contains("emulated")) {
            Toast.makeText(getApplicationContext(), "Error: invalid directory: " + directory, Toast.LENGTH_LONG).show();
            return false;
        }
        return true;
    }
